...
fun parseFSM(
    reader: BufferedReader, alphabets: Alphabet<String>, controllable: Collection<String>,
    observable: Collection<String>
): SupervisoryNFA<Int, String> {
...
fun parseFSM(reader: BufferedReader): SupervisoryNFA<Int, String> {
    ...
    if (isDFA) {
        val builder =
            AutomatonBuilders.newDFA(Alphabets.fromCollection(alphabetsRead.keys)).withInitial(states[0].first)
        ...
        val builder =
            AutomatonBuilders.newNFA(Alphabets.fromCollection(alphabetsRead.keys)).withInitial(states[0].first)
        for (e in stateTransitions) {
            for (trans in e.value)
                builder.from(e.key).on(trans.first).to(trans.second)
        }
